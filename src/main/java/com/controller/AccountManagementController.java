...
public class AccountManagementController {
    ...
    public BaseResponse<String> frozeUser(@RequestBody AccountFroze data){
        ...
        try {
            ...
            if (results.size() == 0) {
                ...
                return BaseResponse.success(null,"修改封禁成功");
            } else {
                return BaseResponse.fail(500, "查询到多条封禁信息");
            }
    ...
    public BaseResponse<Map<String, Object>> getBanUser(@RequestParam String frozeId) {
        if (frozeId == "")
            return BaseResponse.fail(400, "请求参数错误");
        try {
            String sql="select * from account_froze where frozeId = ?";
            List<Map<String, Object>> results = jdbc.queryForList(sql, frozeId);
            if (results.isEmpty()) return BaseResponse.fail( 404,"该账户无不良信息");
            if (results.size() > 1) return BaseResponse.fail(500, "查询到多条封禁信息");
            Map<String, Object> data = results.get(0);
            return BaseResponse.success(data,"无不良行为");
        } catch (DataAccessException e) {
            return BaseResponse.fail(500, "服务器崩溃" + e.getMessage());
        }
    }
    ...
    public BaseResponse<String> updateIsManager(@RequestBody UserInfo data){
